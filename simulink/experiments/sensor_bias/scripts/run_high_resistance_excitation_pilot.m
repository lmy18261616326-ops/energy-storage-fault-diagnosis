%% Cross-operating-point pilot for active high-resistance excitation.
% 32 total runs: four operating points per active mode, two healthy repeats,
% and two Ron magnitudes.  This is the gate before the 448-run expansion.

scriptFolder = string(fileparts(mfilename("fullpath")));
datasetRoot = fullfile(scriptFolder, "dataset_output_v15", ...
    "high_resistance_excitation_pilot");
projectRoot = string(fileparts(fileparts(fileparts(fileparts(scriptFolder)))));
shortWorkRoot = fullfile(projectRoot, ".simtmp", "high_r_excitation_pilot");
base = signal_config();
...
common.cases.randomSeedBase = 815501;
...
runHealthy(common, base, datasetRoot, 1, "mode1_health", "high_r_p_health_m1", 2);
runHighResistance(common, base, datasetRoot, 1, 3, ...
    "switch_S1_high_resistance", "mode1_s1_high_r", "high_r_p_s1");
runHealthy(common, base, datasetRoot, 2, "mode2_health", "high_r_p_health_m2", 2);
runHighResistance(common, base, datasetRoot, 2, 4, ...
    "switch_S2_high_resistance", "mode2_s2_high_r", "high_r_p_s2");

fprintf("High-resistance excitation pilot written to %s\n", datasetRoot);
